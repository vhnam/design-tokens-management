...
export const primitiveTokenSchema = z
  .object({
    tokenName: z.string().min(1, 'Name is mandatory').max(512, 'Name must be less than 256 characters'),
    tokenDotPath: z.string().min(1, 'Dot path is mandatory').max(512, 'Dot path must be less than 256 characters'),
  ...
  .refine((data) => dotPathPattern.test(data.tokenName.trim()), {
    message:
      'Use a non-empty path of segments separated by dots (letters, numbers, underscore, hyphen). Example: color.blue.500 or --color-neutral-100',
    path: ['tokenName'],
  })
  .refine(
    (data) =>
